...
int answer1, answer2;
// Store the total number of sentences which is NOT a pangram in the file
// Store the total number of sentences which is a pangram in the file
...
int main(){
	...
	cout << "the total number of sentences which is a pangram in the file is " << answer2 << endl;
	return 0;
}
...
{
	int m = 0, n = 0, p = 0, q = 0;
	while(a[m]!=0) // if two letter represent same letter erase one
	{
		n = m + 1;
		while (a[n] != 0)
		{
	...
	if (q == 26) // if it contains all letter, return 1
	{
		return 1;
	}
	else // else return 0
